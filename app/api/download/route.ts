...
import { generateDocx, generatePdf, generatePptx } from '@/lib/documents'
...
export async function POST(request: Request) {
  try {
    ...
    const { report, format } = body as { report: Report; format: 'pdf' | 'docx' | 'txt' | 'pptx' }
    ...
    switch (format) {
      case 'pptx':
        ...
        content = await generatePptx(report)
